...
export class AuthService {
    ...
    signUp(email: string, password: string){
      return this.http.post<AuthResponseData>('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyA4frYK0cYpmWPiXQ329pjri6KzK7EpzxU',
      {
        email: email,
        password: password,
    ...
    autoLogout(expirationDuration: number){
      this.expireTimer = setTimeout(this.logOut, expirationDuration);
    }
    ...
    private handleError(errorRes){
      let errorMes = 'An Unknown error occured!'
        if (!errorRes.error || !errorRes.error.error){
          ...
          errorMes = errorRes.error.error.message;
          switch(errorRes.error.error.message){
            case "EMAIL_EXISTS":
              errorMes = "This email is already existed!";
              break;
